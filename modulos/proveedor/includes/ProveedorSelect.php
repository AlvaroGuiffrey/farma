 ...
 *
 * Archivo del include del módulo proveedor que arma una select html con todos
 ...
 *
 * Clase ProveedorSelect del módulo proveedor que permite armar
...
class ProveedorSelect
{
	...
	* los registros de proveedors para representar en la vista.
	*
	* @param $cantidad
	* @param $items array()
	* @param $accion
	...
	*/
	static function armaSelect($cantidad, $items, $accion, $id)
	{
		...
		foreach ($items as $item)
		{
			if ($accion == 'Editar'){
				if ($id == $item['id']){
					$renglon  = '<option value="'.$item['id'].'" selected>'.$item['razon_social'].'</option>'.PHP_EOL;
				}else{
					$renglon  = '<option value="'.$item['id'].'">'.$item['razon_social'].'</option>'.PHP_EOL;
				}
			}else{
				$renglon  = '<option value="'.$item['id'].'">'.$item['razon_social'].'</option>'.PHP_EOL;
			}

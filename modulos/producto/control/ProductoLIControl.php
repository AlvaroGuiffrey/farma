 ...
 */
 /**
  * Modificación para utilizar vectores en el proceso
  * @author     Alvaro Guiffrey
  * @date       17/03/2020
  *
  */
class ProductoLIControl
{
	#Propiedades
	private $_html;
	private $_accion;
	...
	*/
	...
	private function accionControl($oLoginVO)
	{
		...
		$archivoTXT = $_SERVER['DOCUMENT_ROOT'].$_SESSION['dir']."/archivos/litoral.txt";
		...
		switch ($this->_accion){
			 ...
			 * txt con precios de perfumeria y otros (no medicamentos)
			 * los datos dentro de la linea estan separados por ,
			 * importe neto (separado con . decimal)
			 */
				...
				// Totalizadores para control de pruebas
				$cantTitulos = $cantRegistros = 0;
				...
				for( $i = 0; $i < sizeof( $contenido ); $i++) {
					...
					if ($dato[0] < 99998){
						...
						$estado = 1;
						$idArticulo = 0;
						// Carga otros datos necesarios
						$codigoIva = 5; // Precio neto + iva
						$tipoDescuento = 2; // Precio con descuento de Droguería para No Medicamentos
						// Agrega el producto a la tabla productos_prov
				...
				/*
				// Muestras totales de pruebas
				echo "Registros gravados: ".$cantRegistros."<br>";
				echo "Registros NO gravados".$cantTitulos." titulos<br>";
				// Fin muestra totales de pruebas
				*/

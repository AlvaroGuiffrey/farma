...
class PedidoTabla
{
     ...
     * @param $idPedido
     * @param $idProveedor
     * @param $cantPendientes
     * @param $cantPedidos
     * @param $cantUnidades
     * @param $cantEliminados
     * @param $cantDescartados
     * @param $importePedido
     *
     * @return tabla.html (file.html)
     */
    static function armaTabla($items, $cantidad, $cantRenglones, $accion, $razonSocial, $canal, $lote, $cantLotes,
        $idPedido, $idProveedor, $cantPendientes, $cantPedidos, $cantUnidades, $cantEliminados,
        $cantDescartados, $importePedido)
    {
        ...
        fwrite($handle, '<caption>Listado de Pedido ('.$canal.') - '.$razonSocial.'</caption>'.PHP_EOL);
        if ($lote < $cantLotes){
            fwrite($handle, '<caption>Lote # '.$lote.' de '.$cantLotes.' lotes.</caption>'.PHP_EOL);
        }
        // Pie de la tabla
        ...
        $renglon = '<tfoot><tr>
			           <td></td>
                       <td></td>
                       <td>Son: '.$cantRenglones.' productos del lote.</td>
                       <td></td>
                       <td></td>
                       <td></td>
                    </tr>
                    ...
                    <tr>
                       <td></td>
                       <td></td>
                       <td>Pedidos '.$cantPedidos.' Productos.</td>
                       <td></td>
                       <td></td>
                       <td></td>
                    </tr>
                    <tr>
                       <td></td>
                       <td></td>
                       <td>Por $ '.$importePedido.' (sin IVA).</td>
                       <td></td>
                       <td></td>
                       <td></td>
                    </tr>';
        $importePedido = $importePedido + $importe;
        if ($cantidadCero > 0){
            $renglon = $renglon.'<tr>
                            <td></td>
                            <td></td>
        ...
        // hidden para enviar información
        $renglon .= '<input type="hidden" name="razonSocial" value="'.$razonSocial.'">
                     <input type="hidden" name="canal" value="'.$canal.'">
                     <input type="hidden" name="cantidad" value="'.$cantidad.'">
                     <input type="hidden" name="lote" value="'.$lote.'">
                     <input type="hidden" name="cantLotes" value="'.$cantLotes.'">
                     <input type="hidden" name="idPedido" value="'.$idPedido.'">
                     <input type="hidden" name="idProveedor" value="'.$idProveedor.'">
                     <input type="hidden" name="cantPendientes" value="'.$cantPendientes.'">
                     <input type="hidden" name="cantPedidos" value="'.$cantPedidos.'">
                     <input type="hidden" name="cantUnidades" value="'.$cantUnidades.'">
                     <input type="hidden" name="cantEliminados" value="'.$cantEliminados.'">
                     <input type="hidden" name="cantDescartados" value="'.$cantDescartados.'">
                     <input type="hidden" name="importePedido" value="'.$importePedido.'">
                     </tfoot>'.PHP_EOL;
        ...
        If ($cantRenglones==0){
            ...
            foreach ($items as $item)
            {
                ...
                $renglon  = '<tr '.$claseTR.'style="margin:0">
                        ...
                        <td>
                            <input class= "input-mini" type="number" name="cantidadProd['.$item['id'].']" value="'.$item['cantidad'].'" min="1" max="24" style="text-align:center">
                        </td>

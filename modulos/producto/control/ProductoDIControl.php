...
class ProductoDIControl
{
	...
	*/
	...
	private function accionControl($oLoginVO)
	{
		...
		$datos = array();
		$file = new SplFileObject($_SERVER['DOCUMENT_ROOT'].$_SESSION['dir']."/archivos/dispita.csv");
		...
		switch ($this->_accion){
			...
			case 'Iniciar':
				...
				$oDatoVista->setDato('{alertaPeligro}',  'Elimine <b>las TETINAS</b> con precio x cantidad. Actualice TETINAS en PLEX');
			 ...
			 */
				...
				while (!$file->eof()){
					...
					if ($datos[0]>10000){
						...
						$precio = str_replace(",", ".", $datos[3]); // Reemplazamos la coma decimal por el punto decimal
						if ($codigoP == ' ' OR $codigoP == NULL OR $codigoP == 0){ // Registro con codigo del proveedor en blanco
							$contB++;
				...
				//echo "Registros de Producto: ".$contP."<br>";

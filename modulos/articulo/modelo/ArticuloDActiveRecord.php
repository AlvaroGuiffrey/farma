...
Clase::define('ArticuloDVO');

class ArticuloDActiveRecord implements ActiveRecord
{
	...
	public function findAll()
	{
		...
		*/
		...
		try{
			...
			$dbh = DataBase::getInstance();
			//$sth = $dbh->prepare("SELECT * FROM articulos ORDER BY nombre");
			$sth = $dbh->prepare("SELECT * FROM articulos_d");
			$sth->execute();
			/*
			if (!$sth){
				$oPDOe = new PDOException('Problema al intentar consultar datos.', 1);
	...
	public function find($oArticuloVO)
	{
		try{
			$dbh = DataBase::getInstance();
			$sth = $dbh->prepare("SELECT * FROM articulos_d WHERE id=?");
	...
	public function insert($oArticuloVO)
	{
		try{
			$dbh = DataBase::getInstance();
			$sth = $dbh->prepare("INSERT INTO articulos_d
	...
	public function update($oArticuloVO)
	{
		try{
			$dbh = DataBase::getInstance();
			$sth = $dbh->prepare("UPDATE articulos_d
	...
	public function delete($oArticuloVO)
	{
		try{
			$dbh = DataBase::getInstance();
			$sth = $dbh->prepare("DELETE FROM articulos_d WHERE id=?");
			$id = $oArticuloVO->getId();
			$sth->bindParam(1, $id);
			$sth->execute();
			if (!$sth){

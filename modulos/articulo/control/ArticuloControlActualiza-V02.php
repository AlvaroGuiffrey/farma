	...
	private function accionControl($oLoginVO)
	{
		...
		switch ($this->_accion){
			...
			case 'ConfirmarAct':
				...
				$diferenciaMenor = $imprimeRotulo = $imprimeRotuloPromo = $noLeidos = 0;
					...
					foreach ($this->_items as $this->_item){ // extraigo los datos de artículos x artículo
						...
						if (strpos($this->_item['nombre'], 'PR.') !== false) { // Artículo con PROMO
							...
							if ($oProductoModelo->getCantidad()>0){ // tiene producto activo en lista del proveedor de referencia
								...
								if ($this->_margen == 0) $this->_margen = 37;  // margen = 0% pongo 37% de prepo
									...
									if ($this->_precio > $this->_item['precio']) { // PRECIO CON AUMENTO
										...
										if ($this->_porcentajeDiferencia > $this->_diferenciaPrecio) {
											...
											if ($this->_porcentaje > 30) {
											    if (isset($this->_aRangos['r5'])) $this->_modifico = 'S';
											    $linea2 .= " -> MAS DE 30 % ";
												$masTreinta++;
											} elseif ($this->_porcentaje > 20) {
											    if (isset($this->_aRangos['r4'])) $this->_modifico = 'S';
											    $linea2 .= " -> MAS DE 20 % ";
												$masVeinte++;
											} elseif ($this->_porcentaje > 10) {
											    if (isset($this->_aRangos['r3'])) $this->_modifico = 'S';
											    $linea2 .= " -> MAS DE 10 % ";
												$masDiez++;
											} elseif ($this->_porcentaje > 5) {
											    if (isset($this->_aRangos['r2'])) $this->_modifico = 'S';
											    $linea2 .= " -> MAS DE 5 % ";
												$masCinco++;
											} else {
											    if (isset($this->_aRangos['r1'])) $this->_modifico = 'S';
										...
										if ($this->_precio < $this->_item['precio']){
											...
											if ($this->_porcentajeDiferencia > $this->_diferenciaPrecio){
											    ...
											    if (isset($this->_aRangos['r6'])) $this->_modifico = 'S';
												$linea2 .= " -BAJA PRECIO- ";
												$baja++;
											}	
											// Muestra si actualiza

	...
	private function accionControl($oLoginVO)
	{
		...
		switch ($this->_accion){
			...
			case 'Actualizar':
				...
				// ingresa los datos a representar en las alertas de la vista
				$oDatoVista->setDato('{alertaInfo}',  'Actualiza tabla Artículos con tabla Productos, confirme la acción.');
			...
			case 'ConfirmarAct':
				...
				$this->_cantAgregados = $this->_cantBajados = 0;
				$this->_cantActualizados = 0;
				$this->_cont = $this->_cantRegistros = 0;
				
				// ---------------------------------
				// LIMITO A 100 PRODUCTOS Y CORTO PARA PRUEBAS
				//$this->_cantProductos = 100;
				// ----------------------------------
				// lee la tabla productos por lotes 
				while ($this->_cont < $this->_cantProductos){
					$query = "SELECT IDProducto, IDLaboratorio, IDRubro, Codebar, Producto, Presentacion, Activo, Costo, Margen, idTipoIVA, visible
								FROM productos LIMIT ".$this->_cont.", 100";
					$result = mysql_query($query) or die(mysql_error());
					while ($item = mysql_fetch_array($result)){
						...
						if ($oArticuloModelo->getCantidad()==0){ // No existe artículo se AGREGA	
							...
							if (mysql_num_rows($res)>0){ // si existe precio en tabla productoscostos de la DB Plex lo carga
				 ...
				 */	
							...
							if ($item[10]==0) $estadoP = 0; // 0 oculto, 1 visible
							if ($estado != $estadoP){
								$estado = $estadoP;
								$oArticuloVO->setEstado($estado);
								$this->_act = 'S';
				...
				// ---------------------------------
				// LIMITO A 100 ARTICULOS Y CORTO PARA PRUEBAS
				//$this->_cantArticulos = 100;
				//echo "COMIENZO VER BAJAS PLEX -> ".date('Y-m-d H:i:s')."<br>";
				// ----------------------------------
				// lee la tabla articulos por lotes
				while ($this->_cont < $this->_cantArticulos){
				    $this->_items = $oArticuloModelo->findAllLimite($this->_cont, 100);
				    // Prueba
				    //echo "*** Nuevo lote de lectura ***<br>";
				    // --------------
				    if ($oArticuloModelo->getCantidad()>0){ // Si hay artículos continúa
				        foreach ($this->_items as $this->_item){ // extraigo los datos de artículos x artículo
				            $this->_cont++;
				            // para Prueba
				            //echo "... ".$this->_cont." cod: ".$this->_item['codigo']." est: ".$this->_item['estado']."<br>";
				            // ------
				            if ($this->_item['estado'] > 0){
				                // leo plex haber si existe producto
				                $query = "SELECT COUNT(*) 
								                FROM productos 
                                                WHERE IDProducto=".trim($this->_item['codigo']);
				                $result = mysql_query($query) or die(mysql_error());
                                if (mysql_result($result,0) == 0){ // No hay productos en PLEX
                                    $oArticuloVO->setId($this->_item['id']);
                                    $oArticuloModelo->find($oArticuloVO);
                                    $estado = 0; // cambio estado del artículo a 0 (inactivo)
                                    $oArticuloVO->setEstado($estado);
                                    $idUsuarioAct = $oLoginVO->getIdUsuario();
                                    $oArticuloVO->setIdUsuarioAct($idUsuarioAct);
                                    $fechaAct = date('Y-m-d H:i:s');
                                    ...
                                    // --- Muestra bajas para prueba
                                    //echo $this->_cantBajados." -> (".$oArticuloVO->getId().") ".$oArticuloVO->getCodigo()." - BAJADO <br>";
                                    // ------------

<?php
/*
 * Carga por primera vez Dispita como proveedor de referencia para los precios
...
//cargo idProveedor para DISPITA
$idProveedor = 6;
// cargo idUsuarioAct
$idUsuarioAct = 1;
		...
		if ($oArticuloVO->getCodigo()>9999900000){
			...
			if ($oArticuloVO->getCodigoB() > 0){
				...
				if ($oProductoModelo->getCantidad()>0){
					...
					// actualizo siempre tabla articulos
					// lista de precios del proveedor con preferencia para actualizar
					
					$oArticuloVO->setIdProveedor($idProveedor);
					$oArticuloVO->setEquivalencia($equivalencia);
					$oArticuloVO->setIdUsuarioAct($idUsuarioAct);
					$oArticuloVO->setOpcionProv(2);

...
class PedidoControlLotes
{
    ...
    private $_lote;
    private $_cantLotes;
    private $_cantRenglones;
    private $_cantProveedor;
    private $_cantPendientes;
    ...
    private function accionControl($oLoginVO)
    {
        ...
        if (isset($_POST['bt_pedir_s'])) $this->_accion = "PedirL";
        ...
        switch ($this->_accion){
            ...
            case 'Pedir':
                ...
                // datos y lotes en cero
                $this->_idProveedor = $this->_lote = $this->_cantLotes = 0;
                $oDatoVista->setDato('{lote}', $this->_lote);
                $oDatoVista->setDato('{cantLotes}', $this->_cantLotes);
            ...
            case 'PedirL':
                ...
                if (isset($_POST['proveedor'])) $oProveedorVO->setId($_POST['proveedor']);
                $this->_canal = $_POST['canal'];
                $this->_cantLotes = $_POST['cantLotes'];
                $this->_lote = $_POST['lote'];
                //echo "Lotes: ".$_POST['cantLotes']."<br>";
                
                // Si cantidad de lotes es 0, calcula los lotes a procesar
                // si es mayor que cero recibe datos por POST
                if ($this->_cantLotes == 0){
                    // busca datos del proveedor
                    $oProveedorModelo->find($oProveedorVO);
                    $this->_razonSocial = $oProveedorVO->getRazonSocial();
                    $this->_idProveedor = $oProveedorVO->getId();
                    
                    // busca cantidad de pedidos del proveedor
                    $oPedidoVO->setIdProveedor($oProveedorVO->getId());
                    $oPedidoModelo->countPedidosProveedor($oPedidoVO);
                    $this->_cantidad = $oPedidoModelo->getCantidad();
                    // busca cantidad de artículos para pedir reservados al proveedor
                    $oPendienteVO->setIdProveedor($this->_idProveedor);
                    //echo "idProveedor: ".$oPendienteVO->getIdProveedor()."<br>";
                    
                    $oPendienteModelo->countPendientesProvAsignado($oPendienteVO);
                    $this->_cantPendientes = $oPendienteModelo->getCantidad();
                    //$this->_cantPendientes = 366;
                    //echo "cantidad: ".$this->_cantPendientes."<br>";
                    // calcula cantidad de lotes
                    $this->_cantLotes = intval($this->_cantPendientes / 25);
                    if (($this->_cantPendientes % 25) > 0) $this->_cantLotes++;
                    // totalizadores  y otros datos en cero
                    $this->_idPedido = $this->_cantDescartados = $this->_cantUnidades = 0;
                    $this->_cantEliminados = $this->_cantPedidos = $this->_importePedido = 0;
 
                } else { 
                    // recibe totalizadores y otros datos por POST cuando cantLotes > 0
                    $this->_razonSocial = $_POST['razonSocial'];
                    $this->_cantidad = $_POST['cantidad'];
                    $this->_idPedido = $_POST['idPedido'];
                    $this->_idProveedor = $_POST['idProveedor'];
                    $this->_cantPendientes = $_POST['cantPendientes'];
                    $this->_cantDescartados = $_POST['cantDescartados'];
                    $this->_cantUnidades = $_POST['cantUnidades'];
                    $this->_cantEliminados = $_POST['cantEliminados'];
                    $this->_cantPedidos = $_POST['cantPedidos'];
                    $this->_importePedido = $_POST['importePedido'];
                    $oProveedorVO->setId($_POST['idProveedor']);
                }
                ...
                //echo "Proveedor # ".$this->_idProveedor."<br>"; 
                //echo "Cantidad Pendientes p/Pedido: ".$this->_cantPendientes."<br>";
                //echo "Lote #: ".$this->_lote." de ".$this->_cantLotes." lotes <br>";
                // carga el contenido html
                
                // ingresa los datos a representar en el panel de la vista
                if ($this->_cantLotes > 1){
                    $titulo = "# ".$this->_lote;
                } else { 
                    $titulo = "UNICO";
                }
                $oDatoVista->setDato('{tituloPanel}', 'Pedido al Proveedor - Lote: '.$titulo);
                ...
                $this->_items = $oPendienteModelo->findAllProvPedirLote($oPedidoVO, $this->_lote);
                $this->_cantRenglones = $oPendienteModelo->getCantidad();
                ...
                // ordena el array
                $this->_items = ArrayOrdenadoPor::ordenaArray($this->_aPedidos, 'nombre', SORT_ASC);

                PedidoTabla::armaTabla($this->_items, $this->_cantidad, $this->_cantRenglones, $this->_accion, $this->_razonSocial, 
                    $this->_canal, $this->_lote, $this->_cantLotes, $this->_idPedido, $this->_idProveedor,
                    $this->_cantPendientes, $this->_cantPedidos, $this->_cantUnidades, $this->_cantEliminados,
                    $this->_cantDescartados, $this->_importePedido);
                $oCargarVista->setCarga('tabla', '/modulos/pedido/tabla.html');
                break;
            ...
            case 'ConfirmarP':
                ...
                // var_dump($aCantidad);
                // echo "<br>";
                //$oProveedorVO->setRazonSocial($this->_razonSocial);
                ...
                $oDatoVista->setDato('{tituloPanel}', 'Pedido Realizado - Lote: '.$titulo);
                $oDatoVista->setDato('{informacion}', '<p>Totales del pedido realizado al proveedor.</p>
														<p>También puede seleccionar otras acciones</p>
														<p>para los pedidos, ver botones.');
                // carga los eventos (botones)
                if ($this->_lote < $this->_cantLotes){
                    $this->_aEventos = [
                        "pedirConfirmar" => "/modulos/pedido/vista/botonPedirLSeguir.html",
                        "borrar" => "/includes/vista/botonBorrar.html"
                    ];
                $oCargarVista->setCarga('aEventos', $this->_aEventos);
                }
                ...
                // ingresa los datos a representar en las alertas de la vista
                if ($this->_cantLotes > 1){
                    $oDatoVista->setDato('{alertaSuceso}',  'Confirmó Lote: # '.$this->_lote.' de '.$this->_cantLotes.' lotes.');
                } else {
                    $oDatoVista->setDato('{alertaSuceso}',  'Confirmó Lote: UNICO.');
                }
                ...
                if ($this->_lote == 1){
                    $this->_cantPedidos = $this->_cantDescartados = $this->_cantEliminados = 0;
                    $this->_cantUnidades = 0;
                }
                ...
                // arma los datos a representar 
                $oDatoVista->setDato('{cantidad}', $this->_cantidad);
                $oDatoVista->setDato('{razonSocial}', $this->_razonSocial);
                ...
                $oDatoVista->setDato('{cantPendientes}', $this->_cantPendientes);
                ...
                $oDatoVista->setDato('{importePedido}', $this->_importePedido);
                break;
            # ----> acción Ver
            case 'Ver':

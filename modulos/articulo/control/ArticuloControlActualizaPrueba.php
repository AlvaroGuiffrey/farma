 ...
 * de referencia para prueba de modificaciones.
 *
 ...
 */
class ArticuloControlActPrueba
{
    #Propiedades
    private $_html;
    private $_accion;
    ...
    private function accionControl($oLoginVO)
    {
        ...
        switch ($this->_accion){
            ...
            case 'Iniciar':
                ...
                $oDatoVista->setDato('{tituloPanel}', 'Artículos - Actualización de Precios - PRUEBA');
            ...
            case 'Listar':
                ...
                // muestra los array
                var_dump($this->_aProveedores);
                var_dump($this->_aProveedoresLista);
                ...
                $oArticuloVO->setEstado(1); // igual que
                // codigo_b > 0
                $this->_items = $oArticuloModelo->findAllActualizablesPorRubroNombre($oArticuloVO);
                ...
                echo "   (PRUEBA - No actualiza los precios)<br>";
                echo "**************************************<br>";
                echo "Articulos actualizables: ".$topeAgrega."<br>";
                // $topeAgrega = 0;
                $agrega = $condi = 0;
                ...
                if ($oArticuloModelo->getCantidad()>0){ // Si hay artículos actualizables continúa
                    foreach ($this->_items as $this->_item){ // extraigo los datos de artículos x artículo
                        ...
                        echo $cont." #".$this->_item['id']." [".$this->_item['codigo_b']."] <b>".$this->_item['nombre']."</b> - ".$this->_item['presentacion'];
                        $rubro = " ";
                        if ($this->_item['id_rubro'] == 1) $rubro = "<b><FONT COLOR='red'>MED.</FONT></b>";
                        if ($this->_item['id_rubro'] == 2) $rubro = "ACC.";
                        if ($this->_item['id_rubro'] == 3) $rubro = "<b><FONT COLOR='blue'>PERF.</FONT></b>";
                        if ($this->_item['id_rubro'] == 4) $rubro = "VARIOS";
                        // Carga cantidad de rotulos en array $_aCantidadRubros
                        if (isset($this->_aCantidadRubros[$this->_item['id_rubro']])) {
                            $this->_aCantidadRubros[$this->_item['id_rubro']]++;
                        } else {
                            $this->_aCantidadRubros[$this->_item['id_rubro']] = 1;
                        }
                        // Carga cantidad de artículos por proveedor de ref en array $_aCantidadPorProv
                        if (isset($this->_aCantidadPorProv[$this->_item['id_proveedor']])) {
                            $this->_aCantidadPorProv[$this->_item['id_proveedor']]++;
                        } else {
                            $this->_aCantidadPorProv[$this->_item['id_proveedor']] = 1;
                        }
                        // Carga cantidad de artículos por alícuota de IVA en array _aCantidadIva
                        if (isset($this->_aCantidadIva[$this->_item['codigo_iva']])) {
                            $this->_aCantidadIva[$this->_item['codigo_iva']]++;
                        } else {
                            $this->_aCantidadIva[$this->_item['codigo_iva']] = 1;
                        }
                        // Verifica si el artículo tiene rótulo para góndola
                        $impRotulo = "NO";
                        if ($this->_item['rotulo'] > 0) $impRotulo = "SI";
                        echo " < Rubro: ".$this->_item['id_rubro']." - ".$rubro." > /";
                        if ($this->_item['codigo_iva']==5) {
                            echo "(IVA: ".$this->_aCondicionIva[$this->_item['codigo_iva']]."%)";
                        } else {
                            echo "<b><FONT COLOR='red'>(IVA: ".$this->_aCondicionIva[$this->_item['codigo_iva']]."%)</b></FONT>";
                        }
                        echo " / Impr.Rótulo: ".$impRotulo."<br>";
                        // Renglón con datos del costo, proveedor de ref y precio para PLEX
                        echo "<= PLEX => Costo Neto $ ".$this->_item['costo']." (Margen: ".$this->_item['margen']."%) ";
                        echo "<b>Precio Final $".$this->_item['precio']."</b> - Prov: ".$this->_item['id_proveedor']." (".$this->_aProveedores[$this->_item['id_proveedor']].")<br>";
                        /**
                         * Si es PROMO no modifica precio
                         */
                        $pos = strpos($this->_item['nombre'], 'PR.');
                                                    ...
                                                    if ($this->_item['precio'] == $this->_precio){ // Precios iguales no actualiza
                                                        ...
                                                        if ($this->_precio > $this->_item['precio']){
                                                            ...
                                                            if ($this->_porcentajeDiferencia > $this->_diferenciaPrecio){
                                                                ...
                                                                if ($this->_producto['id_proveedor'] == $this->_item['id_proveedor']) {
                                                                    $this->_modifico = 'S';
                                                                    $aumentos++;
                                                            ...
                                                            if ($this->_precio < $this->_item['precio']){
                                                                ...
                                                                if ($this->_porcentajeDiferencia > $this->_diferenciaPrecio){
                                                                    echo " NO ACTUALIZO - DIF $".$this->_diferenciaPrecio." < 2% <br>";
                                                                    $diferenciaMenor++;
                                                                } elseif ($this->_producto['id_proveedor'] == $this->_item['id_proveedor']) {
                                                                    $this->_modifico = 'S';
                                                                    $baja++;
                                                                    echo "<b> ACTUALIZO -BAJA PRECIO- PROV. REF. </b>".$this->_porcentaje."% <br>";
                                                                } else {
                                                                    echo " ACTUALIZO -BAJA PRECIO- OTRO PROV.</b> ".$this->_porcentaje."% <br>";
                                                                }
                                                            }
                                                        }

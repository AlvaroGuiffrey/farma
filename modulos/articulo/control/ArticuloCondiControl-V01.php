...
// >>>>  CODIGO DE PROVEEDOR <<<<<
                        ...
                        if($_POST['codigoProv'] != " "){
                            ...
                            // busca todos los productos activos con igual codigo_p
                            $cantProductos = 0;
                            ...
                            if ($cantProductos == 1) {
                                ...
                                if ($this->_cantidad == 0){ // No encontró artículo por código de barra
                                    $oCargarVista->setCarga('opcion', '/modulos/articulo/vista/buscarArticuloCondi.html');
                                    $oCargarVista->setCarga('alertaPeligro', '/includes/vista/alertaPeligro.html');
                                    $oDatoVista->setDato('{alertaPeligro}',  '<b>No encontró artículo en PLEX</b>.
                                                                            Intente otra búsqueda o AGREGAR en PLEX.<br><'
                                                                            .$this->_item['codigo_b'].'> ['.$this->_item['codigo_p']
                                                                            .'] '.$this->_item['nombre'].' ');
                                } elseif ($this->_cantidad > 1){ // Encontró artículos con el mismo código de barra
                                    $oCargarVista->setCarga('opcion', '/modulos/articulo/vista/buscarArticuloCondi.html');
                                    $oCargarVista->setCarga('alertaPeligro', '/includes/vista/alertaPeligro.html');
                                    $oDatoVista->setDato('{alertaPeligro}',  '<b>PRECAUCION!!! REVISAR PLEX</b>.
                                                                            Código de Barra repetido.<br><'
                                                                            .$this->_item['codigo_b'].'> ['.$this->_item['codigo_p']
                                                                            .'] '.$this->_item['nombre'].' ');
                                } else {  // Encontró un artículo con el código de barra
                                    $oArticuloModelo->findPorCodigoB($oArticuloVO);
                                    $this->_cantidad = $oArticuloModelo->getCantidad();
                                    // revisa si imprime rótulo y avisa
                                    if ($oArticuloVO->getRotulo() == 0){

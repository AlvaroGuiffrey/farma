 ...
 */
/**
 * Modificación para evitar productos duplicados del proveedor.
...
class ProductoKEControl
{
	...
	*/
	...
	private function accionControl($oLoginVO)
	{
		...
		$archivoCSV = fopen ($_SERVER['DOCUMENT_ROOT'].$_SESSION['dir']."/archivos/kellerhoff.csv","r");
		...
		switch ($this->_accion){
			 ...
			 * csv (separado por ;)
			 * "D" y con Neto en 0 (cero) PVP llevan el descuentos de droguería a medicamentos
			 * "D" y "P" con Neto en 1 (uno) PV sin iva, llevan el 5% de descuento
			 *
			 * No inserta renglones con codigo_p en blanco.
			 */
				...
				while ($data = fgetcsv ($archivoCSV, 1000, ";")) {
					...
					$ivaCod = $data[7];
					$estado = 1;
					$precio = str_replace(",", ".", $data[6]);
					if ($data[0]=='P' || $data[0]=='D'){ // Sigo si es un producto
						$contP++;
						if (ctype_space($codigoP) == false){ // Sigo si tiene código de producto del proveedor
						    //echo " -> ".$codigoP." - (".$codigoB.") - ".$nombre." - ".$precio."<br>";
						    if ($ivaCod==1){ // Precio neto + iva
						        $codigoIva = 5;
						        $tipoDescuento = 2; // Precio neto - Desc. Drog. "Productos c/iva"
						    } else { // PVP sin iva
						        $codigoIva = 3;
						        $tipoDescuento = 1; // PVP - Desc. Drog. "Productos s/iva"
						    }
					...
					} // Fin si no es un producto (título, comentario, etc)
				}
				// Cierra archivo csv
				fclose($archivoCSV);
			 ...
			 */
				...
				/*
				// consulto todos los productos activos del proveedor
				$oProductoVO->setIdProveedor($this->_idProveedor);
				$this->_items = $oProductoModelo->findAllPorIdProveedor($oProductoVO);
				if ($oProductoModelo->getCantidad()>0){ // Si tiene productos activos
				    foreach ($this->_items as $this->_item){
				        $oProductoProvVO->setCodigoP($this->_item['codigo_p']);
				        $oProductoProvVO->setIdProveedor($this->_item['id_proveedor']);
				        $this->_aProductos = $oProductoProvModelo->findPorCodigoPProveedor($oProductoProvVO); // solo activos
				        if ($oProductoProvModelo->getCantidad()==0){ // paso a inactivo el producto en tabla productos para el proveedor
				                ...
				                $oProductoModelo->find($oProductoVO);
				                $oProductoVO->setIdArticulo(0); // pongo 0 a prepo
				                $oProductoVO->setEstado(0); // producto con estado inactivo
				                $oProductoVO->setIdUsuarioAct($oLoginVO->getIdUsuario());
				                $this->_date = date('Y-m-d H:i:s');
				                $oProductoVO->setFechaAct($this->_date);
				            ...
				            // Verifico si no modificó el codigo de barra
				            if ($oProductoProvVO->getCodigoB() != $this->_item['codigo_b']){
				                echo "Cambio Codigo Barra<br>";
				                echo "Producto: ".$this->_item['codigo_p']." Cod Barra Ant: ".$this->_item['codigo_b']." Nuevo cod: ".$oProductoProvVO->getCodigoB()."<br>";
				                $oProductoVO->setId($this->_item['id']);
				                $oProductoModelo->find($oProductoVO);
				                $oProductoVO->setIdArticulo(0); // pongo 0 a prepo
				                $oProductoVO->setCodigoB($oProductoProvVO->getCodigoB());
				    ...
				    } // Final foreach de los productos activos del proveedor
				} // Final revisar si proveedor no dio de baja algún producto
				*/

			    // Hora de inicio de Paso5
				//echo "Fin (inicio): ".date('Y-m-d H:i:s')."<br>";
				/*
				$idArticulo = 0;
				$this->_items = $oProductoProvModelo->findAllActivos(); // cargo todos los productos_prov de la lista del proveedor activos
				foreach ($this->_items as $this->_item){
					$oProductoVO->setIdProveedor($this->_idProveedor);
					$oProductoVO->setCodigoP($this->_item['codigo_p']);
					$oProductoModelo->findPorCodigoPProveedor($oProductoVO); // busca producto por id y codigo del proveedor
					if ($oProductoModelo->getCantidad() == 0){ // Agrega un producto
						$oProductoVO->setIdProveedor($this->_idProveedor);
						...
						$oProductoModelo->insert($oProductoVO);
						$this->_cantAgregados++;
					}else{ // verifica si actualiza un producto
